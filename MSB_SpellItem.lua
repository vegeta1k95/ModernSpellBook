-- MSB_SpellItem.lua
-- OOP class for a spell list entry: SpellIcon + name/rank text + trail background.
...
MSB_SpellItem = MSB_Class()

function MSB_SpellItem:Init(parent, index)
    ...
    self.spellIcon = MSB_SpellIcon:New(self.frame)
...
function MSB_SpellItem:Hide()
    self.frame:Hide()
end

function MSB_SpellItem:Show()
    self.frame:Show()
end

function MSB_SpellItem:SetStance(isActive)
    self.spellIcon:SetStance(isActive)
end

-- ============================================================
-- Configuration methods (called from Set)
-- ============================================================

function MSB_SpellItem:SetClickHandler(spellInfo)
...
function MSB_SpellItem:SetTextContent(spellInfo)
    self.nameText:SetFont("Fonts\\FRIZQT__.TTF", ModernSpellBook_DB and ModernSpellBook_DB.fontSize or 11.5)
...
function MSB_SpellItem:SetTextPosition(spellInfo)
    local nameHeight = 13
...
function MSB_SpellItem:SetChatLinkHandler(spellInfo)
...
function MSB_SpellItem:SetTooltipHandler(spellInfo, lookupString, isNew)
...
function MSB_SpellItem:SetCooldownAndDrag(spellInfo)
    local frame = self.frame
...
function MSB_SpellItem:SetLearnedState(spellInfo)
...
function MSB_SpellItem:Set(spellInfo, currentPageRows, page, grid_x)
